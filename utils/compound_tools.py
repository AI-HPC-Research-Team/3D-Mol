    ...
    def get_superedge_angles(edges, atom_poses, dir_type='HT'):
        ...
        def _get_angle(vec1, vec2):
            ...
            if norm1 == 0 or norm2 == 0:
                return 0
            vec1 = vec1 / (norm1 + 1e-9)  # 1e-5: prevent numerical errors
            vec2 = vec2 / (norm2 + 1e-9)
            angle_temp = np.dot(vec1, vec2)
            if angle_temp > 1:
                angle_temp = 1
            if angle_temp < -1:
                angle_temp = -1
        ...
        if len(super_edges) == 0:
            super_edges = np.zeros([0, 2], 'int32')
            bond_angles = np.zeros([0, ], 'float32')
        else:
            super_edges = np.array(super_edges, 'int32')
            bond_angles = np.array(bond_angles, 'float32')
        return super_edges, bond_angles, atom_id_set
...
def getdihes_angle_1(data, conf, atom_id_set):
    ...
    dihes_angle_node_k = []
    dihes_angle_node_l = []
    dihes_angle_node_i_extra = []
    dihes_angle_node_j_extra = []
    dihes_angle_node_k_extra = []
    dihes_angle_node_l_extra = []
    for tar_edge_i in super_edge_indices:
        if atom_id_set[tar_edge_i][0] == atom_id_set[tar_edge_i][2]:
            continue

        tar_edge_atom_pose = atom_id_set[tar_edge_i]

        for src_edge_i in super_edge_indices:
            if atom_id_set[src_edge_i][0] == atom_id_set[src_edge_i][2]:
                continue
            if src_edge_i == tar_edge_i:
                continue
            src_edge_atom_pose = atom_id_set[src_edge_i]

            if src_edge_atom_pose[0] == tar_edge_atom_pose[2] and src_edge_atom_pose[2] == tar_edge_atom_pose[0] and src_edge_atom_pose[1] == tar_edge_atom_pose[1]:
                dihes_edges_0.append([src_edge_i, tar_edge_i])
                dihes_angles_0.append(0)
                ...
                dihes_angle_node_l.append(tar_edge_atom_pose[2])

            elif src_edge_atom_pose[0] != tar_edge_atom_pose[2] and src_edge_atom_pose[1] == tar_edge_atom_pose[1] and src_edge_atom_pose[2] == tar_edge_atom_pose[0]:
                dihes_edges_extra.append([src_edge_i, tar_edge_i])
                angle = 0
                try:
                    angle = rdMolTransforms.GetDihedralDeg(conf, int(src_edge_atom_pose[0]), int(src_edge_atom_pose[1]),
                                                           int(src_edge_atom_pose[2]), int(tar_edge_atom_pose[2]))
                except:
                    print("error")
                dihes_angles_extra.append(angle)
                dihes_angle_node_i_extra.append(src_edge_atom_pose[0])
                dihes_angle_node_j_extra.append(src_edge_atom_pose[1])
                dihes_angle_node_k_extra.append(src_edge_atom_pose[2])
                dihes_angle_node_l_extra.append(tar_edge_atom_pose[2])
            else:
                continue

    dihes_angles = dihes_angles + dihes_angles_0
    dihes_edges = dihes_edges + dihes_edges_0

    if len(dihes_edges) == 0:
        data['dihes_angle_node_i'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_j'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_k'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_l'] = np.zeros([0, ], 'int32')
        data['DihesAngleGraph_edges'] = np.zeros([0, 2], 'int32')
        data['dihes_angle'] = np.zeros([0, ], 'float32')
    else:
        data['dihes_angle_node_i'] = np.array(dihes_angle_node_i, 'int32')
        data['dihes_angle_node_j'] = np.array(dihes_angle_node_j, 'int32')
        data['dihes_angle_node_k'] = np.array(dihes_angle_node_k, 'int32')
        data['dihes_angle_node_l'] = np.array(dihes_angle_node_l, 'int32')
        data['DihesAngleGraph_edges'] = np.array(dihes_edges, 'int32')
        data['dihes_angle'] = np.array(dihes_angles, 'float32')


    if len(dihes_edges_extra) == 0:
        data['dihes_angle_node_i_extra'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_j_extra'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_k_extra'] = np.zeros([0, ], 'int32')
        data['dihes_angle_node_l_extra'] = np.zeros([0, ], 'int32')
        data['dihes_angle_extra'] = np.zeros([0, ], 'float32')
        data['DihesAngleGraph_edges_extra'] = np.zeros([0, 2], 'int32')
    else:
        data['dihes_angle_node_i_extra'] = np.array(dihes_angle_node_i_extra, 'int32')
        data['dihes_angle_node_j_extra'] = np.array(dihes_angle_node_j_extra, 'int32')
        data['dihes_angle_node_k_extra'] = np.array(dihes_angle_node_k_extra, 'int32')
        data['dihes_angle_node_l_extra'] = np.array(dihes_angle_node_l_extra, 'int32')
        data['dihes_angle_extra'] = np.array(dihes_angles_extra, 'float32')
        data['DihesAngleGraph_edges_extra'] = np.array(dihes_edges_extra, 'int32')

    return data


def mol_to_graph_data(mol, total=False, dihes=False, cl=False):
    """
    mol_to_graph_data
    ...
    if len(data['edges']) == 0:  # mol has no bonds
        for name in bond_id_names:
            data[name] = np.zeros((0,), dtype="int32")
        data['edges'] = np.zeros((0, 2), dtype="int32")

    ### make ndarray and check length
    for name in atom_id_names:
        data[name] = np.array(data[name], 'int32')
    data['mass'] = np.array(data['mass'], 'float32')
    for name in bond_id_names:
        data[name] = np.array(data[name], 'int32')
    data['edges'] = np.array(data['edges'], 'int32')

    ### morgan fingerprint
    data['morgan_fp'] = np.array(CompoundKit.get_morgan_fingerprint(mol), 'int32')
    # data['morgan2048_fp'] = np.array(CompoundKit.get_morgan2048_fingerprint(mol), 'int32')
    data['maccs_fp'] = np.array(CompoundKit.get_maccs_fingerprint(mol), 'int32')
    data['daylight_fg_counts'] = np.array(CompoundKit.get_daylight_functional_group_counts(mol), 'int32')
    return data
